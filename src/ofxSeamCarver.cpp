...
void ofxSeamCarver::setup(int width, int height, ofImageType imgType, ofImage inImg) {
    this->img = inImg;
    this->pixelHomes = new ofVec2f*[width];
    
    for(int i = 0; i < width; i++){
        pixelHomes[i] = new ofVec2f[height];
        for(int j = 0; j < height; j++){
            pixelHomes[i][j] = ofVec2f(i,j);
        }
    }
...
void ofxSeamCarver::seamCarve(bool x, bool y) {
...
ofPixels ofxSeamCarver::getPixels(){
...
ofPixels ofxSeamCarver::removeVerticalSeam(ofPixels pixels, float * seamFitness, int w, int h) {
    ...
    ofVec2f** changeHomes = new ofVec2f*[w-1];
    
    for(int i = 0; i < w-1; i++){
        changeHomes[i] = new ofVec2f[h];
    }
    ...
    for (int y = h-1; y >= 0; y--) {
        ...
        for (int x = 0; x < w-1; x++) {
            ...
            changeHomes[x][y] = pixelHomes[skippedColumn ? x+1 : x][y];
        }
...
ofPixels ofxSeamCarver::removeHorizontalSeam(ofPixels pixels, float * seamFitness, int w, int h) {
    ...
    ofVec2f** changeHomes = new ofVec2f*[w];
    
    for(int i = 0; i < w; i++){
        changeHomes[i] = new ofVec2f[h - 1];
    }
    ...
    for (int x = w-1; x >= 0; x--) {
        ...
        for (int y = 0; y < h-1; y++) {
            ...
            changeHomes[x][y] = pixelHomes[x][skippedBestRow ? y+1 : y];
        }
        if (x > 0) {
...
ofVec2f** ofxSeamCarver::getRemainingPixelHomes(){
    return pixelHomes;
}
